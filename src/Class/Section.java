...
public class Section {
    ...
    public Student insert(String first, String last) {
         ...
         * If that fails, we reset the size and id, print our error, and set newStu to be the student already in students
         * then we return our student
         */
        Student newStu = new Student(first, last, "0" + Integer.toString(currID));
        currID++;
        ...
        try {
            ...
            System.out.println(first + last + "is already in section " + sectionNum);
    ...
    public void remove(String first, String last) {
         ...
         * If that fails, restore size, get section number, and print error message
         */
        ...
        try {
            ...
            size += 1;
            int sectionNum = (currID - (size + 1)) / 10000;
            System.out.println("Remove failed. " + first + last + " student doesn't exist in section " + Integer.toString(sectionNum));
        }
    ...
    public void removeSection() {
        //This method just uses the BST makeEmpty method, and resets the size and currID
        students.makeEmpty();
        currID = currID - size;
        size = 0;
     ...
     * @param node the node that is being checked out
     * @param grader the array holding the quantities for each grade
     */
     ...
     * @param name the name we are looking for (can be first or last name)
     * @param node the node we are looking at rn
     * @param sameNames the array of students who have the name (whether first or last
     * @param currSize the current number of students in the sameNames array
     */
    private void findNames(String name, BinaryNode<Student> node, Student[] sameNames, int currSize) {
        //If the left node/subtree exists, we check that out
        if (node.getLeft() != null) {
            this.findNames(name, node.getLeft(), sameNames, currSize);
        }
        //Checks if the current node even exists, then checks if either of its student's names match the given name. if they do, add them to the array
        if (node != null && (node.getValue().getFirstName().equalsIgnoreCase(name) == true || node.getValue().getLastName().equalsIgnoreCase(name) == true)) {
